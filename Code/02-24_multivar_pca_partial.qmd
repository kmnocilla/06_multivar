...
```{r}
weather = read_csv("~/CRSS Stats/05_Opendata/Data/weather_monthsum.csv")
...
lm_dayl.apr = lm(strength_gtex ~ mean_dayl_s_apr,
                 data = weather) %>%
  tidy() %>%
...
lm_prp.oct <- lm(strength_gtex ~ sum_prcp_mm_oct,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
lm_dayl.prcp <- lm(strength_gtex ~ mean_dayl_s_apr + 
                   sum_prcp_mm_oct,
   data = weather) %>%
  tidy() %>%
  filter(term != "(Intercept)") %>%
...
lm_daylapr.daylmay <- lm(strength_gtex ~ mean_dayl_s_apr + 
                  mean_dayl_s_may,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
```{r vif uncorrelated}
# Uncorrelated
vif(lm(strength_gtex))
```


```{r vif correlated}
# Correlated
lm(strength_gtex ~ mean_dayl_s_apr + 
                  mean_dayl_s_may,
   data = weather) %>%
  vif()
